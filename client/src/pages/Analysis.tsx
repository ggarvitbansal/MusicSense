...
import { Play, Loader2, Library, AlertCircle, ArrowLeft } from "lucide-react";
  ...
  const fetchLibrary = async () => {
    ...
    try {
      ...
      setLibraryError(
        ...
        "Failed to load your music library. Make sure you are registered and signed in."
      );
    } finally {
  ...
  const handleAnalyze = async (track: AudioFile) => {
    ...
    try {
      ...
      if (res.data?.success && res.data?.data) {
        ...
        throw new Error("Invalid response format received from analysis endpoint.");
      }
    } catch (err: any) {
      setAnalysisError(
        err.response?.data?.message ||
        "Failed to analyze track. Ensure the ML FastAPI service is online and healthy."
      );
    } finally {
      setAnalyzing(false);
    }
      ...
      <div className="space-y-8 pb-12">
        ...
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs text-gray-500 font-mono bg-gray-950 px-3 py-1 rounded-full border border-gray-800">
            Upload ID: {selectedTrack?.id}
          </span>
        </div>

        {/* Section 1: Song Overview */}
        <SongOverview
          filename={selectedTrack?.originalName || "Unknown Track"}
        ...
        <FeatureSummary
          ...
          harmonicEnergy={metadata.harmonic_energy}
          percussiveEnergy={metadata.percussive_energy}
          silenceRatio={metadata.silence_ratio}
        />
        ...
        <TechnicalDetails
          ...
          contrast={metadata.spectral_contrast}
    ...
    <div className="space-y-6">
      ...
      {loadingLibrary ? (
        ...
        <div className="bg-gray-900/40 border border-gray-800 rounded-xl p-8 text-center max-w-xl min-h-[250px] flex flex-col items-center justify-center mx-auto">
          ...
          <h3 className="text-lg font-bold text-white mb-2">No tracks in library</h3>
          <p className="text-gray-400 text-sm leading-relaxed mb-6">
            Upload audio files first to begin the extraction and analysis pipelines.
          </p>
          <a
